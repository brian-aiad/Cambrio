...
async (page) => {
  ...
  if (await page.locator('.card-flight').count() !== 1) throw new Error('A hand-to-discard swap did not launch its removed card.');
  if (!(await page.locator('.flight-endpoint em').textContent())?.includes('Brian TL')) throw new Error('The removed slot is not labeled during a hand swap.');
  ...
  if (Number(await page.locator('.flight-front').evaluate((element) => window.getComputedStyle(element).opacity)) < 0.9) throw new Error('The removed hidden card did not flip face-up while traveling to discard.');
  if ((await page.locator('.flight-front strong').textContent()) !== 'A') throw new Error('The in-flight discarded face does not match the authoritative discard.');
