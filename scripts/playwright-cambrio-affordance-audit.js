...
async (page) => {
  ...
  for (const players of [2, 8]) {
    await page.goto(`${base}?scene=drawn&players=${players}`);
    await page.waitForTimeout(420);
    ...
    if (!call || !drawn || !deck || !separate(call, drawn)) throw new Error(`Cambrio overlaps the ${players}-player landscape draw panel.`);
    if (!separate(deck, drawn)) throw new Error(`The ${players}-player landscape draw panel obscures the deck: deck=${JSON.stringify(deck)}, drawn=${JSON.stringify(drawn)}.`);
    await page.screenshot({ path: `output/playwright/iteration/drawn-landscape-${players}p-no-overlap.png` });
  }
  ...
  return { startOfTurn: 'callable', duringTurn: 'callable', outOfTurn: 'callable', landscape: 'clear at 2 and 8 players' };
}
